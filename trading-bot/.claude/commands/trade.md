...
1. Pull state: `bash scripts/alpaca.sh account`, `bash scripts/alpaca.sh positions`,
...
2. For BUY, validate:
   ...
   - daytrade_count < 3
   - Catalyst documented (ask for thesis if not in today's memory/RESEARCH-LOG.md)
...
8. Call the PushNotification tool with trade details (one line, ≤200 chars).
